...
public class CommitLog {
        ...
        try {
            ...
            switch (magicCode) {
                case MESSAGE_MAGIC_CODE:
                    break;
                case BLANK_MAGIC_CODE:  // 空白消息，mappedFile 最后的空间不足以存放消息的时候用空白消息填充
            ...
            if (bodyLen > 0) {
                if (readBody) {
                    ...
                    // 没有读取消息内容，但是必须向前推进 byteBuffer 的 position ,下面读取topic length的位置才会准确
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        // 这里由于有锁的保证，所以mappedFile 的创建无需加锁
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        synchronized void putRequest(final GroupCommitRequest request) {
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 由于consume queue 里面的格式是固定大小，每个consume 数据项占20字节， queueOffset按20字节等分的下标
