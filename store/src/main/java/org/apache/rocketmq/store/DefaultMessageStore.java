...
//                    }
    ...
    ConcurrentMap<String, ConcurrentMap<Integer, ConsumeQueue>> getConsumeQueueTable() {
    ...
    void doDispatch(DispatchRequest req) {
    ...
    void unlockMappedFile(final MappedFile mappedFile) {
    ...
    class ReputMessageService extends ServiceThread {
        /**
         * 全局的offset, 跨commit mappedFile文件，意思是可能超过1g
         */
        // 为什么要定义成volatile, 是因为有其他线程需要访问这个值进行一些业务判断，虽然不会写入，但是为了可见性保证，所以要设计为volatile
        private volatile long reputFromOffset = 0;

        long getReputFromOffset() {
            return reputFromOffset;
        }

        void setReputFromOffset(long reputFromOffset) {
        ...
        private void doReput() {
            ...
            // 最小值为0或者，当前有效的mappedFile 的最小有效偏移， 表明reput太慢，并且之前的mappedFile都被删除了
                        ...
                        for (int readSize = 0; readSize < result.getSize() && doNext; ) {
                            ...
                            if (dispatchRequest.isSuccess()) {
                                if (size > 0) {
                                    ...
                                    // 当mappedFile 最后的空间不足以存放一条消息的时候，将会使用一个空白填充消息，消息的size=0
